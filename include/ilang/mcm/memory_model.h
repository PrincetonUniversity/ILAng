...
private:
  ...
  /// A flag to make sure we only traverse the tracestep (inst.decode&update/property) the first time we call FindAddrDataPair
  mutable bool already_searched_for_memreadpair_;
  /// A wrapper of calling DepthFirstVisit with a function object 
  void FindAddrDataPairVecInExpr(const ExprPtr &node, NestedMemAddrDataAvoider & nested_finder_) const;
  ...
  void VisitNodeForMemoryRead(const ExprPtr &node, NestedMemAddrDataAvoider & nested_finder_) const;
  /// For a state, return the reference to the (addr,data) pair list, it has no way to check if sname is memvar or not, so it needs to be guaranteed that it is a memver's name
  const AddrDataVec & FindAddrDataPairVecInInst(const std::string &sname, NestedMemAddrDataAvoider & nested_finder_) const;
...
protected:
  ...
  TraceStepPtr _final_trace_step; // will be assigned by function CreateGlobalFinalStep
...
public:
  ...
  void virtual AddSingleTraceStepProperty( const ExprPtr & property, std::function<bool(const TraceStep &)> filter);
...
private:
  ...
  z3::expr MemVarSameData(
    ...
    const TraceStep & traceL , const TraceStep & traceR, const ExprPtr & memVar) const;
