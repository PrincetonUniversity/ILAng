...
protected:
  ...
  ///
  // --------------------- HELPER FUNC ---------------------------- //
  double virtual _eval(ast_expression* e, const named_parameter_dict_t & param_defs);
  /// parse only the current module's parameter definitions, will update
  /// param_defs
  void ParseCurrentModuleParameters(ast_module_declaration* m, const named_parameter_dict_t & named_parameter_override,
    const ordered_parameter_dict_t & ordered_parameter_override, named_parameter_dict_t & output_parameter_dict);
