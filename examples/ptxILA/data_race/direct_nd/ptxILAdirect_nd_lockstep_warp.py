...
class ptxGPUModel(object):
    ...
    def createPC(self):
        self.pc_list.append(self.model.reg('pc', instruction_format.PC_BITS))
    ...
    def aux_dest(self, opcode, src_list, index):
        ...
        if opcode_name == 'setp':
            ...
            if opcode_cmp == 'lt':
                dest = ila.ite(src_list[0] < src_list[1], self.pred_one, self.pred_zero)
        if opcode_name == 'bar':
            self.bar_inst[index] = self.bar_inst[index] | (self.pc_list[0] == self.current_pc)
    ...
    def perform_instruction(self, index, program_line, pc_target):
            ...
            if (opcode_name != '@') & (opcode_name != 'bra'):
                ...
                self.next_state_dict[dest_str + '_%d' % (index)] = ila.ite(self.pc_list[0] == self.current_pc, ila.ite(conj_pred, dest, current_next_state), current_next_state)
                self.current_pc += 4
                return 
            else:
                ...
                if (opcode_jmp_target > self.current_pc):
                    ...
                    opcode_pred = conj_pred0 | conj_pred1
                pc_jmp = ila.ite(opcode_pred, ila.const(opcode_jmp_target, instruction_format.PC_BITS), self.pc_list[0] + 4)
                if index == 0: 
                    self.pc_next_list[0] = ila.ite(self.pc_list[0] == self.current_pc, pc_jmp, self.pc_next_list[0])
                self.current_pc += 4
    ...
    def set_next_pc(self):
        self.model.set_next('pc' , ila.ite(self.pc_list[0] < (self.pc_max), self.pc_next_list[0], self.pc_list[0]))
    ...
    def add_assumptions(self):
        ...
        for reg in ptx_declaration_diff:
            ...
            self.init = self.init & (reg0 != reg1) & (reg0 <= 32) & (reg0 >= 0) & (reg1 <= 32) & (reg1 >= 0)
        ...
        self.imply = (self.check_register == self.log_register) & (self.en_log_register == 1) & (self.en_check_register == 1) & ((tid0 >> 5) != (tid1 >> 5))
