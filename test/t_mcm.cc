...
TEST(TestTraceStep, ConvertZ3) {
  ...
  // std::cout<< update1z3 << std::endl; // (bvadd OneInstSimp_r2_1 OneInstSimp_r3_1)
  // std::cout<< update2z3 << std::endl; // (bvadd OneInstSimp_r2_2 OneInstSimp_r3_2)
...
TEST(TestMcm, SingleILAUnroll) {
  ...
  u.AddSingleTraceStepProperty( 
  	...
  	[](const TraceStep & ts){return !ts.is_init_tracestep(); }  // a conjunction on every one
  	); 
  u.LinkStates(ordered);
  ...
  u.AddSingleTraceStepProperty( 
  	ExprFuse::Not( ExprFuse::Eq(ila1->state("r0") , ExprFuse::BvConst(0,8) ) ) , 
  	[](const TraceStep & ts){return ts.pos_suffix() == 2; }  // only on the last one
  	); 
  u.LinkStates(ordered);
...
TEST(TestMcm, SingleILAUnrollUnorderedReverse) {
  ...
  InterIlaUnroller::InstrVec inst_seq_;
  InterIlaUnroller::ProgramTemplate tpl_;
  const std::vector<bool> ordered = {false}; // set the ordered attributes to false
...
void TestTsoSb(InstrLvlAbsPtr & T1, InstrLvlAbsPtr &  T2) {
  	...
  	InterIlaUnroller::ProgramTemplate tpl_;
    z3::context c;
    const std::vector<bool> ordered = {true, true}; // set the ordered attributes to false


    ila_vec.push_back(T1);  ila_vec.push_back(T2);
    tpl_.push_back( { T1->instr(0), T1->instr(1) } ); tpl_.push_back( { T2->instr(0), T2->instr(1) } );
    ...
    u.SetFinalProperty( 
        ExprFuse::And( 
            ExprFuse::Eq( T1->state("r1") , ExprFuse::BvConst(0,8) ),
    ...
    u.SetFinalProperty( 
        ExprFuse::And( 
            ExprFuse::Eq( T1->state("r1") , ExprFuse::BvConst(1,8) ),
            ExprFuse::Eq( T2->state("r2") , ExprFuse::BvConst(0,8) )
            ) );
...
void TestTsoMp(InstrLvlAbsPtr & T1, InstrLvlAbsPtr &  T2) {
    ...
    u.SetFinalProperty( 
        ExprFuse::And( 
            ExprFuse::Eq( T2->state("r1") , ExprFuse::BvConst(0,8) ),
            ExprFuse::Eq( T2->state("r2") , ExprFuse::BvConst(0,8) )
            ) );
    ...
    u.SetFinalProperty( 
        ExprFuse::And( 
            ExprFuse::Eq( T2->state("r1") , ExprFuse::BvConst(1,8) ),
            ExprFuse::Eq( T2->state("r2") , ExprFuse::BvConst(0,8) )
            ) );
