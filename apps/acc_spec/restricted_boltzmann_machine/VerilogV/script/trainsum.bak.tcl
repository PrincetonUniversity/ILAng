...
{
    cycle1 |-> (
        GM.train_h_cnt == acc_0.mux_v_ln191_z && // signal match
...
assume -name {onehot} -env \
{
    $onehot0(acc_0.state_rbm_0_cmos32soi_rbm_train_rbm) && $onehot0(acc_0.state_rbm_0_cmos32soi_rbm_train_rbm_next)
}


assume -name {cycle1-start} -env \
{
    cycle1 |-> (nondet_start == 1)
}
...
assert -name {check} \
{
    finish |-> (
        ...
        GM.train_v_cnt == acc_0.mux_h_ln184_z &&
        GM.train_sum == acc_0.mux_sum_ln191_z &&
        (~ GM_hidden_unit_mismatch) // pc->state
