    ...
    for (auto && r : READ_list) {  // forall r : READ_list
      ...
      for (auto && w : WRITE_list )  { // exists w : WRITE_list
        ...
        for (auto && w2 : WRITE_list) {  // forall w2 : WRITE_list
          ...
          ZExpr var3 = Z3Implies( _ctx_.bool_val(true) , z3::implies( ( ( SameAddress ( *r,*w2,s,AxiomFuncHint::HINT_READ,AxiomFuncHint::HINT_WRITE )  && Decode ( *w2 )  )  ) , ( CO ( w2,w )  || FR ( r,w2 )  )  )  );
...
// ------------------------- ACCESSOR FUNCTIONs -------------------------------------- //
MemoryModel::TraceStepPtr Tso::__wfe_global(const MemoryModel::TraceStepPtr & ts)
{
...
InterIlaUnroller::MemoryModelCreator Tso::TsoModel = [] (
          ...
          const InstrLvlAbsPtr & global_ila_ptr
            ...
            return std::unique_ptr<Tso> ( new Tso(ctx, _cstrlist, shared_states, private_states, global_ila_ptr) );
            // mm_make_unique<Tso>( ctx, _cstrlist, shared_states, private_states, global_ila_ptr );
          };
