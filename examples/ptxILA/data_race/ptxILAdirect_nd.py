...
class ptxGPUModel(object):
    ...
    def create_data_race_element(self, access_set, pred_map, log):
        ...
        for pc in access_set.keys():
            ...
            if log:
                self.log_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.arb == 1, access_reg_next, self.log_register_next), self.log_register_next)
                self.en_log_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.arb == 1, en_access_reg_next, self.en_log_register_next), self.en_log_register_next)
            else:
                self.check_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.arb == 1, access_reg_next, self.check_register_next), self.check_register_next)
                self.en_check_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.arb == 1, en_access_reg_next, self.en_check_register_next), self.en_check_register_next)
        if log:
    ...
    def add_assumptions(self):
        ...
        self.imply = (self.check_register == self.log_register) & (en_log_register == 1) & (en_check_register == 1)
...
time_consumption = []
result = []
#print ptx.model.bmcCond(ptx.predicate_bit == ila.bool(True), 15, ptx.init)
