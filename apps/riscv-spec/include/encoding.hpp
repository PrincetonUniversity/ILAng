...
extern unsigned NondetCSRAddr[];
extern unsigned ROCSR[];
extern std::vector<range> CSRNotModeled;

ExprRef nonDetCSRAddr(const ExprRef & idxBits);
ExprRef notModeledCSRAddr(const ExprRef & idxBits);
ExprRef ilegalCSRAddr(const ExprRef & idxBits);
ExprRef indexIntoReadOnlyCSR(const ExprRef & idxBits);
