...
/******************************************************************************/
 ...
 // constructor of an instruction trace step
  TraceStep::TraceStep(const InstrPtr & inst , ZExprVec & cstr, z3::context& ctx , size_t pos  ) : already_searched_for_memreadpair_(false),
  ...
  TraceStep::TraceStep(const InstrPtr & inst , ZExprVec & cstr, z3::context& ctx , ZExpr ts_overwrite ,  size_t pos ) : already_searched_for_memreadpair_(false),
  ...
  TraceStep::TraceStep(const InstrPtr & ref_inst, ZExprVec & cstr , z3::context& ctx, const std::string & s,  size_t pos ) : already_searched_for_memreadpair_(false),
  ...
  TraceStep::TraceStep(const ExprPtr & property, ZExprVec & cstr, z3::context& ctx, const InstrLvlAbsPtr & host): already_searched_for_memreadpair_(false),
    _type(TraceStepType::FINAL_EVT), _cstr(cstr), _name( "__FINAL_STEP__" ), timestamp( ctx.int_const( _name.c_str() ) ) ,
    _pos_suffix(65536) , _expr2z3_(ctx) , _ctx_(ctx), _host_final_(host), _final_property(property)
  {
  ...
  {
    ILA_ASSERT( _type == TraceStepType::FACET_EVT ) << "Cannot change the state access info of non-facet-event trace step: "<< _name ;
    //ILA_ASSERT( _type != TraceStepType::FINAL_EVT ) << "Cannot change the state access info of instruction trace step: "<< _name ;
  ...
  bool TraceStep::Access( AccessType acc_type , const std::string & name) const {
    ILA_ASSERT( _type == TraceStepType::INST_EVT || _type == TraceStepType::INIT_EVT || _type == TraceStepType::FINAL_EVT ) << "Not implemented for facet event";
    switch(acc_type) {
  ...
  std::string TraceStep::Print(z3::model & m) const {
    std::string retStr;
    switch(_type) { 
      case TraceStepType::INST_EVT:  retStr += "[InstrTs]"; break; 
      case TraceStepType::FACET_EVT: retStr += "[FacetTs]"; break;
      case TraceStepType::INIT_EVT:  retStr += "[InitTs] "; break;
      case TraceStepType::FINAL_EVT: retStr += "[FinalTs]"; break;
      default: retStr += "[Type:?]"; }
    retStr += " " + name() + " ";
    retStr += "@";
    retStr += m.eval(timestamp).to_string();
    if(TsPrintDecode && type() != TraceStepType::FINAL_EVT)
      retStr += "   DECODE:" + (m.eval( ConvertZ3OnThisStep( inst()->decode() ) )).to_string();
    ...
    if(TsPrintReadSet) {
      ...
      for (auto sname : _inst_read_set ) {
        retStr += sname;
        if( TsPrintReadVal ) {
          retStr += " : " +  ( m.eval( ConvertZ3OnThisStep( host()->state(sname) ) ) ).to_string();
          retStr += ",\n\t";
        }
        else
          retStr += ", ";
    ...
    if(TsPrintNewVal) {
      if(_type == TraceStepType::FACET_EVT) {
        ...
        for (auto sname : _write_state_set ) 
          retStr += "\n\t" + sname + " : " + m.eval( ConvertZ3OnThisStep( inst()->update(sname) ) ).to_string();
      }
      else {
        retStr += "W set: ";
  ...
  InstrPtr TraceStep::inst() const { 
    ...
    return _type == TraceStepType::INST_EVT || 
           _type == TraceStepType::INIT_EVT ? _inst : _parent_inst; 
  }
...
/******************************************************************************/
  ...
  */
  ...
  // called by specific memory model
  {
    ...
    // but you need to check if it writes some states and you need to add it to the set of tracesteps
    // and this check should be done according to the MCM axioms.
    return _init_trace_step;
  }

  MemoryModel::TraceStepPtr MemoryModel::CreateGlobalFinalStep(const ExprPtr & property)
  // should be called by specific memory model
  {
    _final_trace_step = std::make_shared<TraceStep> (
      property, // const ExprPtr &
      _constr,  // ZExprVec & cstr,
      _ctx_,    // z3::context& ctx,
...
// # error we need to collect last states to used in last property?
  ...
  // link local states
  {
     for(size_t idx = 0; idx != _ila_trace_steps.size(); ++ idx) {
       ...
       ILA_ASSERT( pos != m_ila_private_state_names.end() ) << "Implementation BUG: the private state set should exist for ILA:"<<ila_name;
       ...
       if(is_ordered) {
         ...
         for(auto && ts : ts_seq) {
           ...
           for( auto && sname : private_write_set) {
             auto conditional_update = ExprFuse::Ite( ts->inst()->decode() , ts->inst()->update(sname), ts->host()->state(sname) );
             // NOT using its direct form 
             //  -- last_update_of_a_state[sname] = std::make_pair( ts->inst()->update(sname) , ts->pos_suffix() );
         ...
         for (auto && sname : private_state_name) {
            //ILA_INFO<<"Enumerating:"<<sname<<" (pv state) for ila:"<<(ila_ptr->name().str());
            auto name_expr_pos_pair_ = last_update_of_a_state.find(sname);
        ...
        typedef std::vector< std::tuple<ExprPtr, ExprPtr,  z3::expr, unsigned> > UpdateDecodeTsPosList;
         ...
         ILA_DLOG("MCM.SetLocalState.Unordered") << "==== DEBUG private state: all_update_of_a_state ====";
         for(auto && name_update_tuple_ : all_update_of_a_state) {
          ILA_DLOG("MCM.SetLocalState.Unordered") << name_update_tuple_.first << " : ";
         ...
         for(auto && ts_read : ts_seq) {
           StateNameSet private_read_set;
           INTERSECT( ts_read->_inst_read_set , private_state_name , private_read_set );
           for( auto && sname : private_read_set ) {
             // for this state
             ...
             for (size_t idx_writer = 0; idx_writer != defineList.size() ; ++ idx_writer) {
               ...
               if( pos_ == ts_read->pos_suffix()  ) {  
                ILA_DLOG("MCM.SetLocalState.Unordered") <<"-- Skip writer idx:"<<idx_writer <<" (timestamp: "<<tstamp_ <<")"; 
                continue; } 

               ILA_DLOG("MCM.SetLocalState.Unordered") <<"For writer idx:"<<idx_writer <<" (timestamp: "<<tstamp_ <<")";
               ...
               for ( size_t idx_interfere = 0; idx_interfere != defineList.size() ; ++ idx_interfere) {
                 if(idx_interfere == idx_writer) {
                   ILA_DLOG("MCM.SetLocalState.Unordered") <<"  --skip"<<idx_interfere <<" (timestamp: "<< (std::get<2>(defineList[idx_interfere])) <<")";
                   continue;
                 }
         ...
         for (auto && sname : private_state_name) {
            z3::expr constr_exists_a_writer_ = _ctx_.bool_val(false);
            auto update_map_pos = all_update_of_a_state.find(sname);
            ILA_ASSERT( update_map_pos != all_update_of_a_state.end() ) << "BUG: private state:"<< sname <<" is not defined yet";
            UpdateDecodeTsPosList & defineList = update_map_pos->second;
            for (size_t idx_writer = 0; idx_writer != defineList.size() ; ++ idx_writer) {
               auto & name_expr_pos_tuple_ = defineList[idx_writer];
               auto & expr_   = std::get<0>(name_expr_pos_tuple_);
     ...
     } // for(size_t idx = 0; idx != _ila_trace_steps.size(); ++ idx) (For all entity)
  } // end of function : SetLocalState

  void MemoryModel::AddSingleTraceStepProperty( const ExprPtr & property, std::function<bool(const TraceStep &)> filter)
  {
    GetSingleTraceStepProperty( property, filter , 
    ...
    for (auto && ts : _all_trace_steps) {
      if( filter(*ts) ) {
        ...
        for( auto && sname : read_set ) {
          if(! IN(sname, ts->_inst_read_set) ) { // in case we need to change read set
            // we need to warn users of some possiblility of errors
            if (! ( (ts->type() == TraceStepType::INST_EVT ) || ( ts->type() == TraceStepType::FACET_EVT ) ) )
              ILA_WARN << "Add Property: trying to change the read set of a facet event:" << ts->name() <<" with new state:"<<sname;
            if ( IN(sname, m_shared_state_names) ) 
              ILA_WARN << "Add Property: add READ SHARED state " << sname << " to ts:" << ts->name() <<" does not change the ts SET";
            ILA_ERROR_IF(! ts->host()->state(sname) ) << "Adding a read state: "<< sname 
              <<" that does not belongs to the ILA:" << ts->host()->name().str()
              <<", please consider using a tighter tracestep filter for function: AddSingleTraceStepProperty";
            // if it is just read local, then we have nothing to worry about. we can safely add it
