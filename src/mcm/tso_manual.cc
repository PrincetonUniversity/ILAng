...
TsoTraceStep::TsoTraceStep(const InstrPtr& inst, ZExprVec& cstr,
                           z3::context& ctx, size_t pos)
...
void Tso::RegisterSteps(size_t regIdx, const InstrVec& _inst_seq) {
  ...
  for (auto&& instr_ptr_ : _inst_seq) {
    ...
    auto inst_trace_step_ptr =
        std::make_shared<TsoTraceStep>(instr_ptr_, _constr, _ctx_, pos);
    ...
    if (inst_trace_step_ptr->Access(AccessType::WRITE, m_shared_state_names)) {
      ...
      // now the facet event
      {
        auto _facet_trace_step = std::make_shared<TraceStep>(
            instr_ptr_, _constr, _ctx_, "wfe.global", pos);
        ...
        const StateNameSet& read_set_ =
            inst_trace_step_ptr->get_inst_read_set();
        const StateNameSet& write_set_ =
            inst_trace_step_ptr->get_inst_write_set();
        StateNameSet global_read_set_;
        StateNameSet global_write_set_;
        INTERSECT(read_set_, m_shared_state_names,
                  global_read_set_); // global_read_set_  := read_set_   &
                                     // m_shared_state_names
        INTERSECT(write_set_, m_shared_state_names,
                  global_write_set_); // global_write_set_ := write_set_  &
                                      // m_shared_state_names
    ...
    if (instr_ptr_->name() == "fence" || instr_ptr_->name() == "Fence" ||
...
void Tso::FinishRegisterSteps() {
  ...
  // steps, if there are, make it arbitrary InstructionSet FENCE - skip, name is
  // not FENCE InstructionSet RMW - skip, name not matched
}

void Tso::SetFinalProperty(const ExprPtr& property) {
...
Tso::__wfe_global(const MemoryModel::TraceStepPtr& ts) {
  if (ts->is_init_tracestep())
    return ts;
  Tso::TsoTraceStepPtr _cts = std::dynamic_pointer_cast<TsoTraceStep>(ts);
  ILA_ASSERT(_cts) << "MCM: trace step: " << ts->name()
                   << " has no attribute: wfe.global";
...
z3::expr Tso::RF(const TraceStepPtr& w, const TraceStepPtr& r) {
  if (SameCore(*w, *r))
    return HB(*w, *r);
  if (!SameCore(*w, *r))
    return HB(*__wfe_global(w), *r);
  return (((z3::implies(SameCore(*w, *r), HB(*w, *r))) &&
           (z3::implies(!(SameCore(*w, *r)), HB(*__wfe_global(w), *r)))));
}
...
z3::expr Tso::FR(const TraceStepPtr& r, const TraceStepPtr& w) {
  if (SameCore(*r, *w))
    return HB(*r, *w);
  if (!SameCore(*r, *w))
    return HB(*r, *__wfe_global(w));
  return (((z3::implies(SameCore(*r, *w), HB(*r, *w))) &&
           (z3::implies(!(SameCore(*r, *w)), HB(*r, *__wfe_global(w))))));
}
...
z3::expr Tso::CO(const TraceStepPtr& w1, const TraceStepPtr& w2) {
  if (SameCore(*w1, *w2))
    return HB(*w1, *w2);
  if (!SameCore(*w1, *w2))
    return HB(*__wfe_global(w1), *__wfe_global(w2));
  return (((z3::implies(SameCore(*w1, *w2), HB(*w1, *w2))) &&
           (z3::implies(!(SameCore(*w1, *w2)),
                        HB(*__wfe_global(w1), *__wfe_global(w2))))));
}

// -------------------------- TSO SELECTOR
// -------------------------------------- // because we assume C++11
...
InterIlaUnroller::MemoryModelCreator Tso::TsoModel =
    [](z3::context& ctx,                             // z3 context
      ...
      return std::unique_ptr<Tso>(new Tso(ctx, _cstrlist, shared_states,
                                          private_states, global_ila_ptr));
      // mm_make_unique<Tso>( ctx, _cstrlist, shared_states, private_states,
      // global_ila_ptr );
    };
