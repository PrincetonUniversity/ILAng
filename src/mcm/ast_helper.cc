...
namespace ila {
    ...
    void VarUseFinder<T>::Traverse(const ExprPtr & expr, VarUseList & uses ) {
      ...
      if ( expr->is_op() ) {
        ...
        ILA_NOT_NULL(ptr); // << "Implementation bug: expr type and type name do not match!";
    ...
    void VarUseFinder<T>::Traverse(const InstrPtr & i, VarUseList & uses ) {
      ...
      ILA_NOT_NULL(decode_expr_); // << "The decode function is not set for: instr:"<< i->name().str() << " of ILA: "<< i->host()->name().str();
      Traverse( decode_expr_ , uses);

      Instr::StateNameSet instr_writes_ = i->updated_states(); // get the set of updated state names
      for (auto & state_name_ : instr_writes_) {
        auto update_expr_ = i->update(state_name_);
        ILA_NOT_NULL(update_expr_) ; // << "The update function is NULL for: instr"<< i->name().str() << " of ILA: "<< i->host()->name().str() << " on state " << state_name_;
...
*/
    ...
    /******************************************************************************/
  /*
  void MemReadFinder::FindAddrDataPairVecInTraceStep(const TraceStep & ts, const std::string &sname) {
    if ( ts.is_final_tracestep() )
      FindAddrDataPairVecInExpr( ts.final_property() );
    else // for facet/...
      FindAddrDataPairVecInInst( ts.inst() );
  }
