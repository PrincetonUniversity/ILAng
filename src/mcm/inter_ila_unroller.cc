...
/******************************************************************************/
  ...
  void InterIlaUnroller::createDummyGlobalIla()
  {
    ...
    for (size_t idx = 0; idx != state_num ; ++ idx) {
      ...
      // so when it is referred, the initial value constraints will apply
    }
    ...
    for (auto && name_vlist_pair : temp_shared_states_) {
      ...
      if( vars.size() > 1 ) { // if it has been de
        ...
        if ( vsort->is_bool() ) {
          ...
          global_ila_->NewMemState(name, vsort->addr_width(), vsort->data_width() );
        }
        else 
          ILA_ASSERT(false) << "Implementation bug: Unknown sort for var:"<< name;
        // else we don't know what type it is and cannot create a dummy state of it
  ...
  void InterIlaUnroller::AddSingleTraceStepProperty(const ExprPtr & property, std::function<bool(const TraceStep &)> filter)
  { mm_->AddSingleTraceStepProperty(property, filter);  } // we need to make sure to add state to the read list

  void InterIlaUnroller::GetSingleTraceStepProperty(const ExprPtr & property, std::function<bool(const TraceStep &)> filter,
  ...
  void InterIlaUnroller::GenSysInitConstraints()
  {
    ...
    ILA_WARN_IF( ! CurrConstrSat() ) << "The initial conditions of ILAs are incompatible (unsat)";
  }
  ...
  z3::model & InterIlaUnroller::GetModel()
  {
    ...
    return *(model_ptr_.get()); // dangerous, will only usable before it is destroyed
  }

  const InterIlaUnroller::ZExprVec & InterIlaUnroller::DebugAccessConstrList() const
  {
    return cstr_;
  }
  const InterIlaUnroller::TraceStepPtrSet & InterIlaUnroller::DebugAccessAllTraceStepPtrSet() const
  {
    return mm_->GetAllTraceSteps();
  }

  void InterIlaUnroller::Push()
  {
    zexpr_vec_pos_stack_.push( cstr_.size() );
  }
  void InterIlaUnroller::Pop()
  {
    ILA_ASSERT( !zexpr_vec_pos_stack_.empty() ) << "Unable to pop, inbalanced push/pop";
    size_t prev_size_ = zexpr_vec_pos_stack_.top();
    zexpr_vec_pos_stack_.pop();
    ILA_ASSERT( cstr_.size() >= prev_size_ ) << "Implementation bug, the constraint list should not be shrinked other than pop";
...
void HostRemoveRestore::RestoreAll(InstrLvlAbsPtr h) {
  for (auto & expr_host_pair : map_) {
    ILA_ASSERT(expr_host_pair.first->host() == h) << "Unable to restore to expr:" << expr_host_pair.first 
