...
public:
  ...
  void check_reserved_name() {
    ...
    EXPECT_FALSE(
      vgen.check_reserved_name("__ILA_host_decode_of_NoDecode__"));
...
TEST_F(TestVerilogExport, check_reserved_name) {
  check_reserved_name();
}

TEST_F(TestVerilogExport, sanitizeName) {
  sanitizeName();
}


TEST_F(TestVerilogExport, get_width) {
  get_width();
}

// pass node name, refset
TEST_F(TestVerilogExport, new_id) {
  new_id();
}
// 
TEST_F(TestVerilogExport, dup) {
  dup();
}

TEST_F(TestVerilogExport, death) {
  death();
}


// ?? func -- death
TEST_F(TestVerilogExport, internalfunc) {
  internal_func();
}
...
TEST_F(TestVerilogExport, OPs) {
  ...
  {
    i->set_update(bv_x, 
      ExprFuse::Extract(ExprFuse::Concat(bv_x,bv_y),9,2));
    auto vgen = VerilogGenerator();
    vgen.ExportTopLevelInstr(i);
  }
