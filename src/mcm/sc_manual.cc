...
ScTraceStep::ScTraceStep(const InstrPtr & inst , ZExprVec & cstr, z3::context& ctx , size_t pos )
...
void Sc::RegisterSteps(size_t regIdx , const InstrVec & _inst_seq)
{
  ...
  for (auto && instr_ptr_ : _inst_seq) {
    ...
    auto inst_trace_step_ptr = std::make_shared<ScTraceStep>(instr_ptr_ , _constr, _ctx_ , pos );
...
InterIlaUnroller::MemoryModelCreator Sc::ScModel = [] (
          ...
          const InstrLvlAbsPtr & global_ila_ptr
            ...
            return std::unique_ptr<Sc> ( new Sc(ctx, _cstrlist, shared_states, private_states, global_ila_ptr) );
            // mm_make_unique<Sc>( ctx, _cstrlist, shared_states, private_states, global_ila_ptr );
          };
