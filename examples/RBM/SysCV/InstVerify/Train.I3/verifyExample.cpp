...
__train_i3__END:
		GM.update( conf_done_inp , conf_num_hidden_inp , conf_num_visible_inp , conf_num_users_inp , conf_num_loops_inp , conf_num_testusers_inp , conf_num_movies_inp , rst_inp , rd_grant_inp , data_in_inp , wr_grant_inp );
		 ...
		 __CPROVER_assert( sum == ( GM.train_sum & 0xffff ) / 2.0 , "train_sum_match");
		 __CPROVER_assert( max == ( GM.train_max & 0xffff ) / 2.0 , "train_max_match");
		 __CPROVER_assert( (((unsigned int) (sumOfpow2)) & 0xffffffff) ==  ((unsigned int) (GM.sumOfpow2) & 0xffffffff), "sumOfpow2_match");
...
__train_i3__START:
      ...
      train_upc = 1;
		 ...
		 GM.per_v_pc = 2 ; // __CPROVER_assume( GM.per_v_pc == 2 );
		 done = 0 ; // __CPROVER_assume( done == 0 );
		 GM.train_upc = 1 ; // __CPROVER_assume( GM.train_upc == 1 );
		 ...
		 sum = ( GM.train_sum & 0xffff ) / 2.0 ;
		 max = ( GM.train_max & 0xffff ) / 2.0 ;
       GM.jstate = 4; // or 1
		 j = GM.jstate;
		 v = GM.train_v_cnt;
		 h = GM.train_h_cnt;
       ...
       visibleEnergies.arrayPnt[0] = (GM.visibleEnergies.arrayPnt[0] & 0xffff ) / 2.0;
       visibleEnergies.arrayPnt[1] = (GM.visibleEnergies.arrayPnt[1] & 0xffff ) / 2.0;
       visibleEnergies.arrayPnt[2] = (GM.visibleEnergies.arrayPnt[2] & 0xffff ) / 2.0;
       visibleEnergies.arrayPnt[3] = (GM.visibleEnergies.arrayPnt[3] & 0xffff ) / 2.0;
       visibleEnergies.arrayPnt[4] = (GM.visibleEnergies.arrayPnt[4] & 0xffff ) / 2.0;
		 __CPROVER_array_copy( pow2, GM.pow2.arrayPnt );
		 __CPROVER_array_copy( pos.arrayPnt, GM.pos.arrayPnt );
		// *** END of LABEL: @train.i3@START

       __CPROVER_assume(j<K); //ind-inv
      //__CPROVER_assert(visibleEnergies.arrayPnt[0] == visibleEnergies.arrayPnt[0], "musttrue");
...
__train_i3__END2:
		GM.update( conf_done_inp , conf_num_hidden_inp , conf_num_visible_inp , conf_num_users_inp , conf_num_loops_inp , conf_num_testusers_inp , conf_num_movies_inp , rst_inp , rd_grant_inp , data_in_inp , wr_grant_inp );
		 ...
		 __CPROVER_assert( j == GM.jstate || ( j == K && GM.jstate == 0), "jstate_match");
		 __CPROVER_assert( per_v_pc == GM.per_v_pc , "per_v_pc_match" );
		 __CPROVER_assert( train_upc == GM.train_upc , "train_upc_match" );
		 __CPROVER_assert( v == GM.train_v_cnt , "train_v_cnt_match");
		 __CPROVER_assert( h == GM.train_h_cnt , "train_h_cnt_match");
