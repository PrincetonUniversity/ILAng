...
class repText(object):
    def __init__(self, text, labelList = set([])):
        self.text =text
        # find 
        strlabelList = set(re.findall(r'\{[A-Za-z0-9_]+?\}', self.text )) # not greedy
    ...
    def __add__(self,obj):
        ...
        return repText(self.text + obj.text , labelList = self.labelList | obj.labelList ) # will recalculate label List
    def __radd__(self,lobj):
        if isinstance(lobj,str):
            return repText(lobj+ self.text )
        assert isinstance(lobj,repText)
        return repText(lobj.text + self.text , labelList = lobj.labelList | self.labelList ) 
    ...
    def get(self):
        if not ( len(self.labelList) == 0 or len(re.findall(r'\{[A-Za-z0-9_]+?\}', self.text )) == 0 ):
            print self.text
            print self.labelList
            print re.findall(r'\{[A-Za-z0-9_]+?\}', self.text )
        assert( len(self.labelList) == 0 or len(re.findall(r'\{[A-Za-z0-9_]+?\}', self.text )) == 0 )
        return self.text
    def __str__(self):
        return self.get()
...
def p_axiom(p):
    ...
    p[0] = ('\n// ----- AXIOM %s BEGIN -----\n' % p[2]) + str(body(p[3])) + '\n' + ParseConfig.OutputListName + '.push_back( ' + beg + str(val(p[3])) + end + ( ');\n// ----- AXIOM %s END -----\n\n' % p[2] )
    if TAB:
        p[0] = addTab(p[0])
...
def p_quanexpr_forall(p):
    ...
    extra_restrict = static_determine(expr.simpleInfo.z3e, expr.simpleInfo.z3vlist, True)
...
def p_res_ID(p):
    """res : ID"""
    p[0] = (0, '__' + p[1] + '({varname})') # a condition, which should be converted to a function? like DECODE ?
    ...
    for v in z3vlist:
        ...
        for consq in consqs:
            ...
            if varName in z3vNameToFunc:
                ...
                retStr += 'if (' + funcToCheck + ' ) {res} ' + funcToUse + '; \n'
        s.reset()
        s.add(z3e)
