...
void TraceStep::FindAddrDataPairVecInExpr(const ExprPtr &node,  NestedMemAddrDataAvoider & nested_finder_) const
{
  // read from the map_
  ...
  std::function<void(const ExprPtr &)> func = std::bind(&TraceStep::VisitNodeForMemoryRead, this, std::placeholders::_1 , nested_finder_);
  node->DepthFirstVisit( func  );
}

void TraceStep::VisitNodeForMemoryRead(const ExprPtr &node,  NestedMemAddrDataAvoider & nested_finder_) const
{
  if( node->is_op() ) {
...
const TraceStep::AddrDataVec & TraceStep::FindAddrDataPairVecInInst(const std::string &sname, NestedMemAddrDataAvoider & nested_finder_) const
{
...
z3::expr MemoryModel::MemVarSameAddress(
  ...
  const TraceStep & traceL , const TraceStep & traceR)  const // the last two parameters are just for invoking ConvertZ3OnThisStep
{
...
z3::expr MemoryModel::MemVarSameData(
  ...
  const TraceStep & traceL , const TraceStep & traceR, const ExprPtr & memVar)  const // the last two parameters are just for invoking ConvertZ3OnThisStep
{
...
#define ARG_ADDR 1
#define ARG_DATA 2
ExprPtr MemoryModel::CheckAndPeel(const ExprPtr &e , const std::string & type, size_t argn) const
{
  if ( !e->is_op() )
    return NULL;
  ...
  AxiomFuncHint lhint, AxiomFuncHint rhint ) const
{
  ...
  if ( l.is_init_tracestep() ) return ctx().bool_val(true);
  if ( r.is_init_tracestep() ) return ctx().bool_val(true);
  ...
  ExprPtr leftW; // Future Work: Store(Store(Store())) AST support (multi-store)
  {
    ...
    ExprPtr wexpr = ( lhint == AxiomFuncHint::HINT_READ ) ? NULL : l.inst()->update(sname);
  ...
  ExprPtr rightW;
  {
    ExprPtr wexpr = ( rhint == AxiomFuncHint::HINT_READ ) ? NULL : r.inst()->update(sname);
...
{ 
  ...
  bool leftW = l.Access( AccessType::WRITE, sname ) && lhint != AxiomFuncHint::HINT_READ; // l shoud be an instruction event
  bool leftR = l.Access( AccessType::READ , sname ) && lhint != AxiomFuncHint::HINT_WRITE;

  bool rightW = r.Access( AccessType::WRITE, sname ) && rhint != AxiomFuncHint::HINT_READ; // r shoud be an instruction event
  bool rightR = r.Access( AccessType::READ , sname ) && rhint != AxiomFuncHint::HINT_WRITE;
  ...
  if (leftW && rightR) {
    RWMatchedFlag = true;
    retVal = retVal && ( l.ConvertZ3OnThisStep( l.inst()->update(sname) ) == r.ConvertZ3OnThisStep(var_right) );
  }
  if (rightW && leftR) {
    RWMatchedFlag = true;
    retVal = retVal && ( r.ConvertZ3OnThisStep( r.inst()->update(sname) ) == l.ConvertZ3OnThisStep(var_left) );
  }

  if( !RWMatchedFlag ) {
    if (leftW && rightW) {
      retVal = retVal && ( l.ConvertZ3OnThisStep( l.inst()->update(sname) ) == r.ConvertZ3OnThisStep( r.inst()->update(sname) ) );
    }
    if (leftR && rightR) {
  ...
  AxiomFuncHint lhint, AxiomFuncHint rhint) const
{
  ...
  // now we need to decide how to pair the two 
  return MemVarSameData( {leftWAddr, leftWData}, leftR, {rightWAddr, rightWData}, rightR , l, r, mem_var_global );
}
...
void MemoryModel::SameAddrDataSanityCheck( const TraceStep & l, const TraceStep & r , const std::string & sname ) const {
  ...
  switch( l.type() ) {
    case TraceStepType::FACET_EVT: 
      UNION(l._read_state_set , l._write_state_set, left );  break;
    case TraceStepType::INST_EVT : 
    case TraceStepType::INIT_EVT :
    case TraceStepType::FINAL_EVT:
      UNION(l._inst_read_set  , l._inst_write_set,  left );  break; // it should be the instruction,  not the facet event
    default:
  ...
  switch( r.type() ) {
    case TraceStepType::FACET_EVT: 
      UNION(r._read_state_set , r._write_state_set, right );  break;
    case TraceStepType::INST_EVT :
    case TraceStepType::INIT_EVT :
    case TraceStepType::FINAL_EVT:
      UNION(r._inst_read_set  , r._inst_write_set,  right );  break;
    default:
      ILA_ASSERT(false) << "TraceStepType is neither facet event or instruction event.";
  }
  ...
  for(auto & state_name : intersect) {
    ...
    if( left_state_ptr->sort() == right_state_ptr->sort()  ) // they need to have the same type (sort)
