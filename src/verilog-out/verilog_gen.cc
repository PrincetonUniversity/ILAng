...
VerilogGenerator::VerilogGenerator (const VlgGenConfig & config, const std::string &modName,const std::string &clk,const std::string &rst)
  : moduleName(modName)
  , clkName(clk)
  , rstName(rst)
  , idCounter(0)
...
bool VerilogGenerator::check_reserved_name(const vlg_name_t & n) const {
  if(n == moduleName || n == clkName || n == rstName || n == validName || n == counterName)
    return false;
...
void VerilogGenerator::addInternalCounter( vlg_name_t decode_sig_name , size_t width ) {
  ...
  vlg_stmt_t ctr_stmt = 
    "if ( " + decode_sig_name + " ) begin \n           "+counterName+ " <= 1; end\n       " +
    "else if( " + inc_cond + ") begin\n           " + counterName + " <= "+ counterName +" + 1; end";
...
VerilogGenerator::vlg_name_t VerilogGenerator::getVlgFromExpr(const ExprPtr & e) {
  auto pos = nmap.find(e);
  ILA_ASSERT( pos != nmap.end() ) << "Expr:"<< (e) << " has not been translated yet";
  return pos->second;
}
VerilogGenerator::vlg_name_t VerilogGenerator::getArg(const ExprPtr & e, const size_t & i ) {
  auto arg_i = e->arg(i);
...
VerilogGenerator::vlg_name_t VerilogGenerator::translateBoolOp( const std::shared_ptr<ExprOp> & e ) {
  vlg_stmt_t result_stmt;
...
// will be used by ParseNonMemUpdateExpr, will not be directly called by ParseMemUpdateNode
// the later will call the former first
VerilogGenerator::vlg_name_t VerilogGenerator::translateBvOp( const std::shared_ptr<ExprOp> & e ) {
  vlg_stmt_t result_stmt;
  std::string op_name = e->op_name();
  size_t arg_num = e->arg_num();
...
void VerilogGenerator::ParseNonMemUpdateExpr( const ExprPtr & e ) { // will be used in parsing state update of non mem and decode function
  ...
  if( e->is_bool() ) {
    ...
    else ILA_ASSERT(false) << "Expr sort: "<< ( e->sort() ) << " is not supported." ;
...
void VerilogGenerator::ParseMemUpdateNode( const ExprPtr & cond, const ExprPtr & e , const std::string & mem_var_name) { 
...
void VerilogGenerator::DumpToFile(std::ostream & fout) const {
  ...
  for(auto const &sig_pair : mem_o)
    fout << "output "<<std::setw(10)<<WidthToRange(sig_pair.second)<<" "<<(sig_pair.first) << ";\n";
  for(auto const &sig_pair : regs)
    fout << "output reg " <<std::setw(10)<<WidthToRange(sig_pair.second)<<" "<<(sig_pair.first) << ";\n";
  ...
  for(auto const & mem : mems_internal) // mems.first is just a name
    fout << "reg "<<std::setw(10)<<WidthToRange( std::get<2>(mem.second) )<<" "<<( std::get<0>(mem.second) )<< WidthToRange(std::pow(2, std::get<1>(mem.second)))<<";\n";
