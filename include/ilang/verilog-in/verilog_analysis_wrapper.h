...
/// \brief VerilogAnalyzerBase should never be instantiated, only used as a pointer type in class VerilogInfo
...
public:
  ...
  typedef enum {NONE = 0, 
    MODULE,                 /*1*/
    I_WIRE_wo_INTERNAL_DEF, /*2*/ // module (input [3:0] a, output reg b) ; no internal defininition
    O_WIRE_wo_INTERNAL_DEF, /*3*/
    IO_WIRE_wo_INTERNAL_DEF,/*4*/
    I_WIRE_w_INTERNAL_DEF,  /*5*/ // module (a,b,c); input c **OR** module (input [3:0] a) ; wire [3:0] a;
    O_WIRE_w_INTERNAL_DEF,  /*6*/
    IO_WIRE_w_INTERNAL_DEF, /*7*/
    O_REG_wo_INTERNAL_DEF,  /*8*/
    O_REG_w_INTERNAL_DEF,   /*9*/
    REG /*10*/, WIRE /*11*/, OTHERS /*12*/} hierarchical_name_type;
...
public:
  ...
  static bool is_reg (hierarchical_name_type tp_) { return tp_ == O_REG_wo_INTERNAL_DEF || tp_ == O_REG_w_INTERNAL_DEF || tp_ == REG; }
  /// decide if the type is a wire (port w or wo internal def / internal)
  static bool is_wire(hierarchical_name_type tp_) { return ( tp_ >= I_WIRE_wo_INTERNAL_DEF && tp_ <= IO_WIRE_w_INTERNAL_DEF ) || tp_ == WIRE; }
  ///  decide if a type has no internal def
  static bool no_internal_def(hierarchical_name_type tp_) {
    return (tp_>=I_WIRE_wo_INTERNAL_DEF && tp_<=IO_WIRE_wo_INTERNAL_DEF) || tp_ == O_REG_wo_INTERNAL_DEF;
  }
  /// decide if a type is module
  static bool is_module(hierarchical_name_type tp_) { return tp_ == hierarchical_name_type::MODULE;}
  /// decide if it is port signal
  static bool is_io_sig(hierarchical_name_type tp_) { return tp_ >= I_WIRE_wo_INTERNAL_DEF && tp_ <= O_REG_w_INTERNAL_DEF ;}
  /// decide if it is input signal
  static bool is_input(hierarchical_name_type tp_) {return (tp_ == I_WIRE_wo_INTERNAL_DEF) || (tp_ == I_WIRE_w_INTERNAL_DEF); }
  /// decide if it is output signal
  static bool is_output(hierarchical_name_type tp_) { return is_io_sig(tp_) && ! is_input(tp_); }
  /// Print  location info
  static std::ostream & PrintLoc (std::ostream &os, const vlg_loc_t &loc) { return (os<<(loc.first)<<":"<<(loc.second)); }
...
public:
  ...
  virtual std::string get_hierarchical_name() const  { return _hierarchical_name; }
public:
  /// --------------------- ACCESSORS ------------------- ///
  ...
  _name(n),_hierarchical_name(h), _width(w), _type(typ), _loc(loc)
  {}
...
public:
  ...
  /// (should not be used!) because you don't know what to convert
  /// left here for debugging purpose only
