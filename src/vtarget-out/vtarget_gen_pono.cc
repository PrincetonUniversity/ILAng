...
void VlgSglTgtGen_Pono::Export_problem(const std::string& yosys_script_name) {
  if (_problems.assertions.empty() && _problems.smt_assertions.empty() &&
      _problems.sanity_assertions.empty()) {
  ...
  // sanity checks
  if (!_problems.sanity_assertions.empty()) { // export to ys_script_name
    std::string ys_script_name_path =
  ...
  ILA_CHECK(
    ...
    ys_script_fout << "write_btor " << write_btor_options << " sanity.btor2"
                   << std::endl;
  } // finish writing
} // only for Pono
