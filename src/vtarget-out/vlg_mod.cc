...
VerilogModifier::VerilogModifier(VerilogInfo * _vlg_info_ptr) : vlg_info_ptr( _vlg_info_ptr) {}
...
void VerilogModifier::ReadModifyWrite(const std::string & fn, std::istream & fin, std::ostream & fout ) {
  ...
  while (std::getline(fin, line)) {
    ...
    // keeps
    while (keep_vec_it != keep_info.end() and lineno == std::get<0>(*keep_vec_it)) {
      // process it
    ...
    while (assign_vec_it != assign_info.end() and lineno == std::get<0>(*assign_vec_it) ) {
    ...
    while (mod_decl_vec_it != mod_decl_info.end() and lineno >= std::get<0>(*mod_decl_vec_it) ) {
...
void VerilogModifier::RecordConnectSigName(const std::string & vlg_sig_name, const std::string & suffix) {
  ...
  for( unsigned idx = 1; idx < hier - 1 ; ++ idx ) {
    ...
    ILA_ASSERT(loc.first != ""); // should be found
    mod_inst_map[loc.first].push_back( mod_inst_item_t(loc.second, vname, width) );
  }
...
std::string VerilogModifier::add_assign_wire_to_this_line( const std::string & line_in, const std::string & vname, unsigned width, const std::string & short_name) {
...
bool VerilogModifier::add_mod_decl_wire_to_this_line( const std::string & line_in, std::string & line_out, const std::string & vname, unsigned width) {
...
bool VerilogModifier::add_mod_inst_wire_to_this_line( const std::string & line_in, std::string & line_out, const std::string & vname, unsigned width) {
