/// \file Verilog Verification Target Generator -- generating CHC target to enhance invariant
...
void VlgSglTgtGen_Chc_wCNF::PreExportProcess() {
  ...
  for (auto&& pbname_prob_pair : _problems.assumptions) {
    ...
    ILA_DLOG("VlgSglTgtGen_Chc_wCNF.PreExportProcess") << "Adding assumption:" << prbname;
...
void VlgSglTgtGen_Chc_wCNF::Export_script(const std::string& script_name, const std::string& cnf_name) {
...
std::shared_ptr<smt::YosysSmtParser> VlgSglTgtGen_Chc_wCNF::GetDesignSmtInfo() const {
  return design_smt_info;
}

void VlgSglTgtGen_Chc_wCNF::ExportAll(const std::string& wrapper_name, // wrapper.v
...
void VlgSglTgtGen_Chc_wCNF::Export_script(const std::string& script_name) {
  ILA_ASSERT(false) << "Deprecated. Should not be used!";
}
