...
/*MOD*/	 num_hidden.write( GM.num_hidden );
...
LOAD_FIRST_INPUT:
   ...
   do { wait(); 
__rdGrantInst__START__0:
		 rd_grant_inp = 1 ; // __CPROVER_assume( rd_grant_inp == 1 );
		 rd_request = 1 ; // __CPROVER_assume( rd_request == 1 );
		 GM.rd_request = 1;
		...
		// *** END of LABEL: @rdGrantInst@START@0
  }
   while(!rd_grant.read());
   rd_request.write(false);
...
__rdGrantInst__END__0:
		GM.update( conf_done_inp , conf_num_hidden_inp , conf_num_visible_inp , conf_num_users_inp , conf_num_loops_inp , conf_num_testusers_inp , conf_num_movies_inp , rst_inp , rd_grant_inp , data_in_inp , wr_grant_inp );
     	...
     	__CPROVER_assert( i < nv , "ind-inv");
		return;
		// *** END of LABEL: @rdGrantInst@END@0
