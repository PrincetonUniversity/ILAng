...
class ptxGPUModel(object):
    ...
    def createMonitor(self):
        ...
        self.monitor_arb_fun_list = [self.model.fun('monitor_arb_fun_0', 1, []), self.model.fun('monitor_arb_fun_1', 1, [])]
        self.monitor_arb_list = [ila.appfun(self.monitor_arb_fun_list[0], []), ila.appfun(self.monitor_arb_fun_list[1], [])]
    ...
    def createBar(self):
        ...
        self.bar_arb_next = ila.ite(((self.bar_state_list[0] == self.bar_spec.BAR_WAIT) & (self.bar_state_list[1] == self.bar_spec.BAR_WAIT)) | ((self.bar_state_list[0] != self.bar_spec.BAR_WAIT) & (self.bar_state_list[1] != self.bar_spec.BAR_WAIT)), ila.appfun(self.bar_arb_fun, []), ila.ite(self.bar_state_list[1] == self.bar_spec.BAR_WAIT, self.model.const(0x0, 1), self.model.const(0x1, 1)))
    ...
    def create_data_race_element(self, access_set, pred_map, log):
        ...
        for pc in self.bar_list:
            ...
            for bar_pred_name in bar_pred_list:
                ...
                bar_pred = bar_pred & (bar_pred_reg == self.pred_zero)
        ...
        for pc in access_set.keys():
            ...
            if log:
                #self.log_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.monitor_arb_list[index] == 1, access_reg_next, self.log_register_next), self.log_register_next)
                self.log_register_next = ila.ite((self.pc_list[index] == pc) & (self.monitor_arb_list[index] == 1), access_reg_next, self.log_register_next)
                #self.en_log_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.monitor_arb_list[index] == 1, en_access_reg_next, self.en_log_register_next), self.en_log_register_next)
                self.en_log_register_next = ila.ite((self.pc_list[index] == pc) & (self.monitor_arb_list[index] == 1), en_access_reg_next, self.en_log_register_next)
 
            else:
                self.check_register_next = ila.ite((self.pc_list[index] == pc) & (self.monitor_arb_list[index] == 1), access_reg_next, self.check_register_next)
                self.en_check_register_next = ila.ite((self.pc_list[index] == pc) & (self.monitor_arb_list[index] == 1), en_access_reg_next, self.en_check_register_next)
 
                #self.check_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.monitor_arb_list[index] == 1, access_reg_next, self.check_register_next), self.check_register_next)
                #self.en_check_register_next = ila.ite(self.pc_list[index] == pc, ila.ite(self.monitor_arb_list[index] == 1, en_access_reg_next, self.en_check_register_next), self.en_check_register_next)
        if log:
    ...
    def aux_generate_src_list(self, program_line, index, op_len):
        src_list = []
    ...
    def generate_next_state(self, index):
        ...
        for program_line in program:
            ...
            self.perform_instruction(index, program_line)
    ...
    def set_next_bar(self):
        ...
        #print self.bar_counter_enter_next
        #print self.bar_counter_exit_next
        for i in range(self.thread_num):
            #print self.bar_state_next_list[i]
    ...
    def add_assumptions(self):
        ...
        for reg in ptx_declaration_diff:
            ...
            self.init = self.init & (reg0 != reg1) & (reg0 <= thread_id_range_max) & (reg0 >= thread_id_range_min) & (reg1 <= thread_id_range_max) & (reg1 >= thread_id_range_min)
