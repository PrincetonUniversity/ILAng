...
public:
  ...
  /// \param[in] a property that is going to translated to Z3, note: the state are treated as prestate
  ...
  /// \brief Get the z3 expression of a property on a trace step, so you can play with it
  /// \param[in] a property that is going to translated to Z3, note: the state are treated as prestate
  /// \param[in] a filter to choose which trace step to enforce
  /// \param[in] a collector (callback function) to do something as each time a property is created on a step
  void GetSingleTraceStepProperty( const ExprPtr & property, std::function<bool(const TraceStep &)> filter,
  ...
  /// \brief Check and potentially set the model: return true if sat , false o.w.
  bool CheckSat();
  /// \brief Get the model: the reference will only be usable if unroller is not destroyed
  z3::model & GetModel();
  /// \brief Get access to expr vector, will only be usable if unroller is not destroyed, only for DEBUG use
...
private:
  ...
  MemoryModelPtr mm_;
  /// The model of the 
  std::unique_ptr<z3::model> model_ptr_;// should not shared with any one, you need to assume it will destroy along with the unroller
