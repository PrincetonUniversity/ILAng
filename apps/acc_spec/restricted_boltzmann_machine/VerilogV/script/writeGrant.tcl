...
assume -name {RBM_write_grand_cond} -env \
{
    finish_signal == ( acc_0.state_rbm_0_cmos32soi_rbm_store[6] == 1 )
} -type {temporary} -update_db;

assume -name {RBM_write_grand_start} -env \
{
    nondet_start |-> ( acc_0.state_rbm_0_cmos32soi_rbm_store[4] == 1 )
} -type {temporary} -update_db;


assume -name {RBM_write_grand_assume} -env \
{  start |-> ( 
    conf_done == 0 && 
    rst == 0 && 
    acc_0.init_done == 1 && 
...
assume -name {RBM_write_grand_step} -env \
{
	( GM_step == 1  ) |=> finish
} -type {temporary} -update_db;

assert -name {RBM_write_grand_check} \
{ \
    finish |-> ( \
    GM.num_hidden == acc_0.num_hidden && \
    GM.num_loops == acc_0.num_loops &&  \
