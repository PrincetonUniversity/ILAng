      ...
      output_done.write(false);
{
      ...
      do { wait(); 
__wrGrantInst__START:
		 wr_request = 1 ; // __CPROVER_assume( wr_request == 1 );
		 wr_grant_inp = 1 ; // __CPROVER_assume( wr_grant_inp == 1 );
		...
		// *** END of LABEL: @wrGrantInst@START
  }
      while(!wr_grant.read());
      wr_request.write(false);
