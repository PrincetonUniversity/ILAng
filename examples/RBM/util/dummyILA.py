# This is a dummy one
# Its responsibility is to track the information of a template
import cPickle
import operator
import subprocess
import os
...
def Abstraction(name):
    ...
    parent_list[name] = None
    return fsm_macro
def saveAll(fname):
    for name, Abs in fsm_list.items():
        for n,w in Abs.state_list:
            if n in Abs.port:
                continue
            if n not in Abs.setNextedStatesList:
                parentName = parent_list[name]
                flag = False
                while parentName:
                    if n not in fsm_list[parentName].pure_stateList and n not in fsm_list[parentName].mem_list:
                        break
                    if n in fsm_list[parentName].setNextedStatesList:
                        flag = True
                        break
                    parentName = parent_list[parentName]
                if not flag:
                    print '<W>: %s.%s has not been set next expr!' % (name,n)
...
def exitHandler():
    ...
    print 'Parsing template \t\t[done] \nResults in "template_parse_result".'
...
class collector:
    ...
    def get_microabstraction(self,name):
        return fsm_list[name]
    def add_microabstraction(self,name,valid):
        ...
        ucollector.setNextedStatesList = copy.deepcopy(self.setNextedStatesList)

        self.ufsm[name] = valid
        fsm_list[name] = ucollector

        parent_list[name] = self.name
        return ucollector
    ...
    def set_next(self,name,*args,**kwargs):
        for (n,w) in self.state_list:
            if name == n:
                if name not in self.setNextedStatesList:
                    self.setNextedStatesList.append(name)
                return None
        for (n,a,d) in self.mem_list:
            if name == n:
                if name not in self.setNextedStatesList:
