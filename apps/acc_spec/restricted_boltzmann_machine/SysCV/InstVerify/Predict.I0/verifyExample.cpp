...
// visibleEnergies is ignored for FixPoint
       ...
       __CPROVER_assert(v<=nv, "ind-inv");
       __CPROVER_assert(h<nh, "ind-inv");
		return;
		// *** END of LABEL: @predict.i0@END
...
__predict_i0__START:
       ...
       train_input_done = 0 ; // __CPROVER_assume( predict_input_done == 0 );
		 predict_input_done = 1 ; // __CPROVER_assume( predict_input_done == 0 );
		 GM.predict_upc = 0 ; // __CPROVER_assume( GM.predict_upc == 0 );
       ...
       __CPROVER_assume(v<=nv);
       __CPROVER_assume(h<nh);
		// *** END of LABEL: @predict.i0@START



         u8 current_data = data[pingpong&0][v];
         wait();
         if(current_data == 1)
         {
            tmp = edges[v][h];
