...
// I have to put this include here, because it needs to know the class VerilogAnalyzerBase
...
public:
  ...
  SignalInfoPort (ast_port_declaration *def, const std::string & full_name , VerilogAnalyzerBase::hierarchical_name_type tp ) ;
...
public:
  /// Constructor: from ast_port_declaration
  SignalInfoReg (ast_reg_declaration *def, const std::string & full_name, VerilogAnalyzerBase::hierarchical_name_type tp) ;
  /// Return its definition
...
public:
  /// Constructor: from ast_port_declaration
  SignalInfoWire (ast_net_declaration *def, const std::string & full_name, VerilogAnalyzerBase::hierarchical_name_type tp) ;
  /// Return its definition
...
protected:
  ...
  /// static instance count, we must make sure there is only one instance of it at a time
  /// o.w. it will mess up w. the C code
...
public:
  ...
  static std::ostream & PrintMeta(std::ostream &os, const ast_metadata &md) { return (os<<(md.file)<<":"<<(md.line)); }
  /// Print the meta of an Ast node (cannot be void* , has to be converted)
  template <class T> static std::ostream & PrintMetaAst(std::ostream &os, const T *n) { return PrintMeta(os,n->meta); }
  /// Convert ast_meta to location
  static vlg_loc_t Meta2Loc(const ast_metadata &md) { return vlg_loc_t(md.file, md.line); }
