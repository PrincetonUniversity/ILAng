...
private:
  ...
  static ExprRef GetCipherUpdate_MidRound(const ExprRef & state_in, const ExprRef rnd, const ExprRef & key);
  /// Get ciphertext update for the final round
  static ExprRef GetCipherUpdate_FinalRound(const ExprRef & state_in, const ExprRef rnd, const ExprRef & key);
