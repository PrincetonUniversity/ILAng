...
__load__END__0:
       ...
       __CPROVER_assert( i < nv , "ind-inv");  // It is very to specify inductive invariant even in micro-to-micro prove
                                               // because they are not modelel exactly the same way 
		return;
		// *** END of LABEL: @load@END@0
...
__load__START__0:
		 ...
		 GM.rd_granted = 1 ; // __CPROVER_assume( GM.rd_granted == 1 );
		 GM.rd_request = 1 ; // __CPROVER_assume( GM.rd_request == 1 );
		 rst_inp = 0 ; // __CPROVER_assume( rst_inp == 0 );
		 GM.rd_complete = 0 ; // __CPROVER_assume( GM.rd_complete == 0 );
       ...
       __CPROVER_assume( i < nv ); // ind-inv
		// *** END of LABEL: @load@START@0
