/// \file The definition of AES 128 function
///  Hongce Zhang (hongcez@princeton.edu)

#include "aes_128.h"

#include <ilang/cpp_api.h>
#include <cassert>

unsigned S [256] = {  };

unsigned xS [256] = {};

unsigned rcon[10] = {0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};


using namespace ilang;

ExprRef AES_128::rcon(const ExprRef & rnd) {
  assert(rnd->bit_width() == 4);

  ExprRef ret = BvConst(rcon[0], 8);
  for(int i = 1; i < 256; i ++)
    ret = Ite(rnd == i, rcon[i], ret);
  return ret;
}

ExprRef AES_128::s_table_read(const ExprRef & idx) {
  assert(idx->bit_width() == 8);

  ExprRef ret = BvConst(S[0], 8);
  for(int i = 1; i < 256; i ++)
    ret = Ite(idx == i, S[i], ret);
  return ret;
} 


ExprRef AES_128::xs_table_read(const ExprRef & idx) {
  assert(idx->bit_width() == 8);

  ExprRef ret = BvConst(xS[0], 8);
  for(int i = 1; i < 256; i ++)
    ret = Ite(idx == i, xS[i], ret);
  return ret;
} 


ExprRef AES_128::combine_vec4(const AES_128::vec4 & l) {
  return Concat( Concat ( Concat( l[0], l[1] ) , l[2] ) , l[3] );
}

AES_128::vec4 AES_128::slice_128_to_32(const ExprRef & bv) {
  assert(bv->bit_width() == 128);

  vec4 ret;
  for (int bidx = 3; bidx >= 0 ; bidx -- ) 
    ret.push_back( bv( 32*bidx+31 , bidx*32 ));
  return ret;
}

AES_128::vec4 AES_128::slice_32_to_8(const ExprRef & _32) {
  assert(_32->bit_width() == 128);

  vec4 ret;
  for (int bidx = 3; bidx >= 0 ; bidx -- ) 
    ret.push_back( _32( bidx*8+7 , bidx*8 ));
  return ret;
}


ExprRef AES_128::S(const ExprRef & inp) {
  return s_table_read(inp);
}
ExprRef AES_128::xS(const ExprRef & inp) {
  return xs_table_read(inp);
}
ExprRef AES_128::S4(const ExprRef & inp) { // inp is 32bits
  assert(inp->bit_width() == 32);

  auto ret = S(inp(31,24));
  ret = Concat(ret, S(inp(23,16)) );
  ret = Concat(ret, S(inp(15, 8)) );
  ret = Concat(ret, S(inp( 7, 0)) );

  return ret;
}

AES_128::vec4 AES_128::T(const ExprRef & inp) {  // inp: 8
  assert(inp->bit_width() == 8);
  auto sl0 = S(inp);
  auto sl1 = sl0;
  auto sl3 = xS(inp);
  auto sl2 = sl1 ^ sl3;

  return {sl0,sl1,sl2,sl3};
}

ExprRef AES_128::expand_key_128_a( const ExprRef & in , const Expr & rc) {
  assert(in->bit_width() == 128);
  assert(rc->bit_width() == 8);

  auto K = slice_128_to_32(in);
  auto v0 = Concat( K[0](31,24) ^ rc , K[0](23,0) );
  auto v1 = v0 ^ K[1];
  auto v2 = v1 ^ K[2];
  auto v3 = v2 ^ K[3];

  return combine_vec4({v0,v1,v2,v3});
}


ExprRef AES_128::expand_key_128_b( const ExprRef & in , const Expr & rc) {
  assert(in->bit_width() == 128);
  assert(rc->bit_width() == 8);

  auto K = slice_128_to_32(in);
  auto v0 = Concat( K[0](31,24) ^ rc , K[0](23,0) );
  auto v1 = v0 ^ K[1];
  auto v2 = v1 ^ K[2];
  auto v3 = v2 ^ K[3];

  auto k0a = v0;
  auto k1a = v1;
  auto k2a = v2;
  auto k3a = v3;

  auto k4a = S4( Concat( K[3](23:0) , K[3](31:24) ) )

  auto k0b = k0a ^ k4a;
  auto k1b = k1a ^ k4a;
  auto k2b = k2a ^ k4a;
  auto k3b = k3a ^ k4a;

  return combine_vec4({k0b,k1b,k2b,k3b});
}


AES_128::vec4 AES_128::table_lookup(const ExprRef &s32) { // s32: 32 bit wide
  auto b = slice_32_to_8(s32);
  auto rl = T(b[0]);
  auto p0 = combine_vec4( rl[3], rl[0], rl[1], rl[2] );
  rl = T(b[1]);
  auto p1 = combine_vec4( rl[2], rl[3], rl[0], rl[1] );
  rl = T(b[2]);
  auto p2 = combine_vec4( rl[1], rl[2], rl[3], rl[0] );
  rl = T(b[2]);
  auto p3 = combine_vec4( rl[0], rl[1], rl[2], rl[3] );

  return {p0,p1,p2,p3};
}



ExprRef AES_128::GetCipherUpdate_MidRound(const ExprRef & state_in, const ExprRef rnd, const ExprRef & key) {
  auto enc_key = expand_key_128_a( key , rcon(rnd) );
  auto K0_4 = slice_128_to_32(enc_key);
  auto S0_4 = slice_128_to_32(state_in);

  auto p0 = table_lookup(S0_4[0]);
  auto p1 = table_lookup(S0_4[1]);
  auto p2 = table_lookup(S0_4[2]);
  auto p3 = table_lookup(S0_4[3]);

  auto z0 = p0[0] ^ p1[1] ^ p2[2] ^ p3[3] ^ K0_4[0];
  auto z1 = p0[3] ^ p1[0] ^ p2[1] ^ p3[2] ^ K0_4[1];
  auto z2 = p0[2] ^ p1[3] ^ p2[0] ^ p3[1] ^ K0_4[2];
  auto z3 = p0[1] ^ p1[2] ^ p2[3] ^ p3[0] ^ K0_4[3];

  return combine_vec4({z0,z1,z2,z3});
}


ExprRef AES_128::GetCipherUpdate_FinalRound(const ExprRef & state_in, const ExprRef rnd, const ExprRef & key) {
  auto enc_key = expand_key_128_a( key , rcon(rnd) );
  auto K0_4 = slice_128_to_32(enc_key);
  auto S0_4 = slice_128_to_32(state_in);

  auto p0 = S4(S0_4[0]);
  auto p1 = S4(S0_4[1]);
  auto p2 = S4(S0_4[2]);
  auto p3 = S4(S0_4[3]);

  auto z0 = combine_vec4({p00, p11, p22, p33}) ^ k0;
  auto z1 = combine_vec4({p10, p21, p32, p03}) ^ k1;
  auto z2 = combine_vec4({p20, p31, p02, p13}) ^ k2;
  auto z3 = combine_vec4({p30, p01, p12, p23}) ^ k3;

  return combine_vec4({z0,z1,z2,z3});
}




ExprRef AES_128::GetKeyUpdate_MidRound(const ExprRef & key, const ExprRef rnd) {
  return expand_key_128_b( key , rcon(rnd));
}

AES_128::AES_128 () {
  auto RndModel = Ila("AES_128_Rnd");

  plaintext = RndModel.NewBvInput("plaintext" ,128);
  key_in    = RndModel.NewBvInput("key_in"    ,128);

  ciphertext= RndModel.NewBvState("ciphertext",128);
  round_key = RndModel.NewBvState("round_key"   ,128);

  round     = RndModel.NewBvState("round",   4); // maximum 10 round

  Rnd.AddInit ( round == 0 );
  Rnd.SetValid( round < 10 ); // 0-9 , all bvs are treated as unsigned number by default

  // initial round

  { // FirstRound : just an xor
    auto instr = RndModel.NewInstr("FirstRound");

    instr.SetDecode( round == 0 );
    
    instr.SetUpdate( ciphertext, round_key ^ plaintext );
    instr.SetUpdate( round_key,  round_key );
    instr.SetUpdate( round, round + 1 );

  }

  // intermediate round

  { // Midround
    auto instr = RndModel.NewInstr("IntermediateRound");

    instr.SetDecode( (round > 0) & (round < 9) );

    instr.SetUpdate( ciphertext, GetCipherUpdate_MidRound(ciphertext, round, round_key ) );
    instr.SetUpdate( round_key,  GetKeyUpdate_MidRound(round_key, round) );
    instr.SetUpdate( round, round + 1 );

  }

  { // FirstRound
    auto instr = RndModel.NewInstr("FinalRound");

    instr.SetDecode( (round == 9) );
    
    instr.SetUpdate( ciphertext, GetCipherUpdate_FinalRound(ciphertext, round, round_key ) );
    // we don't specify it as it does not matter
    // instr.SetUpdate( round_key,  unknown(128) );
    instr.SetUpdate( round, round + 1 );

  }


}