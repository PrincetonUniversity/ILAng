...
class ptxGPUModel(object):
    ...
    def create_data_race_element(self, access_set, pred_map, log):
        ...
        for pc in self.bar_list:
            ...
            self.log_clean = self.log_clean | ((self.pc_list[index] == pc) & (conj_bar_pred))
            self.check_clean = self.check_clean | ((self.pc_list[index] == pc) & (conj_bar_pred))
        ...
        for pc in access_set.keys():
            ...
            if log:
                self.log_register_next = ila.ite((self.pc_list[index] == pc) & (self.arb_list[index] == 1), access_reg_next, self.log_register_next)
                self.en_log_register_next = ila.ite((self.pc_list[index] == pc) & (self.arb_list[index] == 1) & (conj_pred), en_access_reg_next, self.en_log_register_next)
                ...
                self.check_register_next = ila.ite((self.pc_list[index] == pc) & (self.arb_list[index] == 1), access_reg_next, self.check_register_next)
                self.en_check_register_next = ila.ite((self.pc_list[index] == pc) & (self.arb_list[index] == 1) & (conj_pred), en_access_reg_next, self.en_check_register_next)
    ...
    def perform_instruction(self, index, program_line, pc_target):
            ...
            if opcode_name != '@':
                ...
                self.next_state_dict[dest_str + '_%d' % (index)] = ila.ite(self.pc_list[index] == self.current_pc, ila.ite(conj_pred, dest, current_next_state), current_next_state)
                self.current_pc += 4
                return 
            else:
                ...
                if (opcode_jmp_target > self.current_pc):
                    ...
                    opcode_pred = conj_pred0 | conj_pred1
                pc_jmp = ila.ite(opcode_pred, ila.const(opcode_jmp_target, instruction_format.PC_BITS), self.pc_list[index] + 4)
