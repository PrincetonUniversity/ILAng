...
IlaSimTest::IlaSimTest()
  ...
  { // WRITE_ADDRESS
    ...
    instr.SetDecode(
      (cmd == CMD_WRITE) & (cmdaddr >= ADDR) & (cmdaddr < ADDR + 2) );

    instr.SetUpdate(address,
      Ite(is_status_idle, 
          Concat(        
            Ite(cmdaddr == ADDR + 1, cmddata, address(15,8) ), 
  ...
  { // START_ENCRYPT
    ...
    instr.SetDecode((cmd == CMD_WRITE) & (cmdaddr == START) &
                    (cmddata == 1));
    ...
    { // LOAD
      ...
      child_instr.SetUpdate(status,
                    Ite(byte_cnt == 15, BvConst(STATE_OPERATE, 2), status));
    }
    ...
    { // store
      ...
      child_instr.SetUpdate(status,
                    Ite(byte_cnt == 15,
                        Ite(blk_cnt + 16 < length, 
                            BvConst(STATE_READ_DATA, 2), 
